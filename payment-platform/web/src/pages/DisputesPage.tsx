...
export function DisputesPage() {
  ...
  const Row = ({
    ...
    const dispute = paginatedItems[index];
      ...
      <div style={style} className="px-1">
        <div className="card flex items-start sm:items-center justify-between gap-2 sm:gap-4 p-3 sm:p-4">
          ...
          <div className="flex-shrink-0">
            <StatusChip status={dispute.status} type="dispute" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-lg sm:text-xl font-semibold">Disputes</h1>
      <div className="card p-3 sm:p-4">
        <form onSubmit={onCreate} className="grid gap-3">
          ...
          />
          <button className="btn w-full sm:w-auto">Open Dispute</button>
        </form>
      </div>
      {error && (
